...
function renderResults(data) {
  ...
  // § BAR FIX v12 — DEFINITIVE
  // The element has style="width:0%" in HTML so the baseline is always 0.
  // We only set className (color). After results become visible we use
  // double-rAF: first frame acknowledges display:block, second frame
  // triggers the CSS transition from 0% → score%. No hacks needed.
  ...
  bar.style.width = '0%';   // explicit reset in case of re-scan
  ...
  // Double-rAF: first rAF fires when display:block is committed to layout,
  // second rAF fires when the browser has actually painted it.
  // At that point setting width triggers the CSS transition cleanly.
...
function exportReport() {
  ...
  const lines = [
    '='.repeat(60), '  SECURITY ASSESSMENT REPORT', '  EOL EOL & CVE Checker  v12 CVE Checker  v11', '='.repeat(60), '',
    ...
    '='.repeat(60),
    'Generated by EOL & CVE Checker v12 — https://theeolchecker.pages.dev',
    '='.repeat(60),
