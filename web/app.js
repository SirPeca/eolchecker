...
async function doScan() {
  ...
  // Reset bar animation so it re-fires on next scan
  const barEl = $('riskScoreBar');
  if (barEl) { barEl.classList.remove('bar-animated'); barEl.style.width = '0%'; }
  hideBoxes();
...
function renderResults(data) {
  ...
  // § BAR FIX v13 — KEYFRAME INJECTION (display:none safe)
  // CSS @keyframes animation — fires on class-add, immune to display:none.
  // CSS @keyframes animations fire on class-add, AFTER display:block,
  // so they are immune to the display:none timing problem entirely.
  // We inject a named @keyframes rule with the exact target width,
  // then add .bar-animated to trigger it.
  const score = data.risk.score || 0;
  $('riskScore').textContent = score;
  const bar = $('riskScoreBar');
  const barClass = riskClass(data.risk.level).replace('risk-','bar-');
  // Reset: remove animation class, reset width, set color class
  ...
  // Inject @keyframes with exact target width, then add .bar-animated class.
  // Animations fire on class-add after display:block — 100% reliable.
  const styleEl = document.getElementById('bar-kf') || document.createElement('style');
  styleEl.id = 'bar-kf';
  styleEl.textContent = '@keyframes barFill { from { width: 0% } to { width: ' + score + '% } }';
  if (!styleEl.parentNode) document.head.appendChild(styleEl);

  // Small timeout ensures the class removal+re-add cycle resets the animation
  setTimeout(() => {
    bar.classList.add('bar-animated');
    // After animation completes, set width directly so it stays on re-paint
    setTimeout(() => { bar.style.width = score + '%'; }, 1100);
  }, 20);
...
function exportReport() {
  ...
  const lines = [
    '='.repeat(60), '  SECURITY ASSESSMENT REPORT', '  EOL EOL & CVE Checker  v13 CVE Checker  v11', '='.repeat(60), '',
    ...
    '='.repeat(60),
    'Generated by EOL & CVE Checker v13 — https://theeolchecker.pages.dev',
    '='.repeat(60),
