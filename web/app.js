...
async function doScan() {
  ...
  // Reset bar for re-scan — clear inline style so CSS 0% is the baseline
  const barReset = $('riskScoreBar');
  if (barReset) barReset.style.width = '';
  hideBoxes();
...
function renderResults(data) {
  ...
  // § BAR — v17
  // .results uses opacity/visibility transition (not display:none).
  // bar.style.width='' lets CSS default 0% be the baseline.
  // setTimeout(50) fires after a real paint, triggering the CSS transition.
  ...
  bar.style.width = '';    // clear any previous inline width — CSS default is 0%
  ...
  // § v17 FIX: setTimeout(50) guarantees TWO real paint frames.
  // Frame 1 (immediate): results becomes visible, browser paints bar at 0% (CSS default).
  // Frame 2 (50ms later): bar.style.width changes 0%→score%, CSS transition fires.
  // rAF alone is not enough — browser can batch it into the same paint as classList.add.
  // 50ms is above the typical 16ms frame budget, guaranteeing a separate paint.
...
function exportReport() {
  ...
  const lines = [
    '='.repeat(60), '  SECURITY ASSESSMENT REPORT', '  EOL EOL & CVE Checker  v17 CVE Checker  v11', '='.repeat(60), '',
    ...
    '='.repeat(60),
    'Generated by EOL & CVE Checker v17 — https://theeolchecker.pages.dev',
    '='.repeat(60),
