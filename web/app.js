...
function renderResults(data) {
  ...
  // § BAR FIX v16 — CLEAN CSS TRANSITION
  // Root cause of all previous failures: .results had display:none + animation:fadeIn
  // CSS animations on a parent block child transitions during their run.
  // Fix: .results now uses opacity:0/1 + visibility transition (no display toggle,
  // no animation). The bar element is always in the layout tree.
  // Result: simple CSS transition on .risk-bar-fill works perfectly.
  ...
  bar.style.width = '0%';  // reset to 0 before making visible
  ...
  // Make results visible (opacity transition — no display:none toggle)
  $('results').classList.add('visible');

  // CSS transition fires cleanly because .results is always in layout.
  // Single rAF gives the browser one frame to process the width:0% reset,
  // then we set the target width and the CSS transition animates it.
...
function exportReport() {
  ...
  const lines = [
    '='.repeat(60), '  SECURITY ASSESSMENT REPORT', '  EOL EOL & CVE Checker  v16 CVE Checker  v11', '='.repeat(60), '',
    ...
    '='.repeat(60),
    'Generated by EOL & CVE Checker v16 — https://theeolchecker.pages.dev',
    '='.repeat(60),
...
function clearForm() {
  ...
  const barClr = $('riskScoreBar');
  if (barClr) barClr.style.width = '0%';
