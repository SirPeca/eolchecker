...
function renderResults(data) {
  ...
  // Make results visible (opacity 0→1 transition)
  $('results').classList.add('visible');

  // § v18 FIX — transitionend listener
  // opacity:0 (no visibility:hidden) keeps bar in layout at all times.
  // We wait for the opacity transition to FINISH (transitionend),
  // then set bar width. At that point the element is fully painted
  // and the CSS width transition fires 100% reliably.
  // {once:true} auto-removes the listener after first fire.
  $('results').addEventListener('transitionend', function(e) {
...
function exportReport() {
  ...
  const lines = [
    '='.repeat(60), '  SECURITY ASSESSMENT REPORT', '  EOL EOL & CVE Checker  v18 CVE Checker  v11', '='.repeat(60), '',
    ...
    '='.repeat(60),
    'Generated by EOL & CVE Checker v18 — https://theeolchecker.pages.dev',
    '='.repeat(60),
